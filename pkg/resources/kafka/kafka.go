...
func (r *Reconciler) createStandardExternalListenerStatuses(log logr.Logger, eListener banzaiv1beta1.ExternalListenerConfig) (banzaiv1beta1.ListenerStatusList, error) {
	...
	for iConfigName, iConfig := range ingressConfigs {
		...
		} else if eListener.GetAccessMethod() == corev1.ServiceTypeLoadBalancer && r.KafkaCluster.Spec.GetIngressController() != envoygatewayutils.IngressControllerName {
		...
		if eListener.GetAccessMethod() != corev1.ServiceTypeNodePort && r.KafkaCluster.Spec.GetIngressController() != envoygatewayutils.IngressControllerName {
