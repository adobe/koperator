...
func requireJmxMetrics(kubectlOptions k8s.KubectlOptions) {
	...
	ginkgo.It("When kraft mode is enabled, brokers/controllers should have kafka_server_raft_metrics_current_state metric available", func() {
		if kRaftEnabled {
			checkMetricExistsForBrokers(kubectlOptions, kafkaLabelSelectorBrokers, "kafka_server_raft_metrics_current_state", true)
			checkMetricExistsForBrokers(kubectlOptions, kafkaLabelSelectorControllers, "kafka_server_raft_metrics_current_state", true)
		} else {
			checkMetricExistsForBrokers(kubectlOptions, kafkaLabelSelectorBrokers, "kafka_server_raft_metrics_current_state", false)
		}
	})
}
