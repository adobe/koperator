...
// and patches only that field back - leaving every other field (including the surviving brokers' own
// per-broker config) untouched. Scaling tests previously triggered an add/remove by applying a second,
// independently maintained sample manifest for the desired broker count; besides the add/remove itself, that
// manifest can silently carry unrelated drift (e.g. simplekafkacluster.yaml's headlessServiceEnabled: false,
// which raced the operator's headless Service teardown against an in-flight broker removal - see #316).
